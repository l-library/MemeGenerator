...
void CyberDistressingDialog::applyWaterMark()
{
    ...
    // 随机选择用户名
    for (int i = 0; i <= m_watermark / 4; ++i)
    {
        int index = QRandomGenerator::global()->bounded(array.size());
        drawWaterMark(array.at(index).toString());
    }
    need_to_change_mark = false;
}

void CyberDistressingDialog::drawWaterMark(QString name)
{
    QString user_name = "@" + name;
    if (m_filteredImage.isNull() || user_name.isEmpty())
    {
        return;
    }
    // 设置绘制器
    ...
    // 设置字体
    QFont font("宋体", std::min(m_filteredImage.width(), m_filteredImage.height()) * 0.030); // 动态调整字体大小
    font.setBold(true);
    painter.setFont(font);

    // 设置水印颜色（半透明白色）
    QColor watermarkColor(255, 255, 255, 150); // RGBA，A=150表示半透明
    painter.setPen(watermarkColor);

    // 计算文本尺寸
    QFontMetrics metrics(font);
    int textWidth = metrics.horizontalAdvance(user_name);
    int textHeight = metrics.height();

    // 获取图像尺寸
    int imageWidth = m_filteredImage.width();
    int imageHeight = m_filteredImage.height();

    // 获取随机位置
    MarkPosition position = static_cast<MarkPosition>(QRandomGenerator::global()->bounded(4));
    ...
    // 添加阴影效果增强可读性
    painter.setPen(QColor(0, 0, 0, 100)); // 黑色半透明阴影
    painter.drawText(drawPosition + QPoint(2, 2), user_name);

    // 绘制水印文本
    painter.setPen(watermarkColor);
    painter.drawText(drawPosition, user_name);
